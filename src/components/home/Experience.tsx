        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Link
              ...
              className="text-muted-foreground/50 hover:text-foreground transition-colors inline-flex no-underline"
              aria-label={`Visit ${exp.displayName ?? exp.company} website`}
            >
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Link
              ...
              onClick={(e) => e.stopPropagation()}
              className="text-muted-foreground/50 hover:text-foreground transition-colors inline-flex no-underline"
              aria-label={`View ${exp.displayName ?? exp.company} on GitHub`}
            >
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Link
              ...
              onClick={(e) => e.stopPropagation()}
              className="text-muted-foreground/50 hover:text-foreground transition-colors inline-flex no-underline"
              aria-label={`View ${exp.displayName ?? exp.company} on LinkedIn`}
            >
            ...
            <article key={exp.company} className="py-5 sm:py-6 first:pt-0">
              ...
              >
                <div className="flex items-start gap-3 sm:gap-4 min-w-0">
                  <Image
                    src={exp.links.image}
                    alt={`${exp.displayName ?? exp.company} logo`}
                    width={44}
                    height={44}
                    loading="lazy"
              ...
              {/* The outcome, not just the job title - the single most scannable
                  thing on the row, so it renders by default while the chevron
                  holds the detailed bullets.

                  It sits OUTSIDE the header row on purpose. Nested inside that
                  row's left column it was a flex sibling of the fixed-width
                  date block, so its measure was (row - logo - gap - dates - gap)
                  - roughly 380px of a 608px row. Every one-liner wrapped at
                  about 60% of the width with a void to its right. As a sibling
                  it gets the full row, and pl-[3.75rem] (44px logo + 16px gap)
                  lines it up with both the company name above it and the
                  expanded bullets below. */}
              <p
                className={`mt-2 text-sm leading-relaxed text-foreground/80 sm:pl-[3.75rem] ${
                  !completeView ? "cursor-pointer" : ""
                }`}
                onClick={!completeView ? () => toggle(exp.company) : undefined}
              >
                {exp.oneLiner}
              </p>
