...
//
// Workers-safe by construction: a plain `fetch` to a public host, no Node APIs,
// no module-scope state. Every failure path - network error, non-2xx, malformed
// body - returns null rather than throwing, so a third-party outage degrades to
// a one-line message instead of taking the homepage down. The `revalidate` hint
// is honoured because open-next.config.ts wires a real R2 incrementalCache;
// without that override it would silently refetch on every request.
    ...
    <div className='w-full max-w-full overflow-hidden px-4 sm:px-6 md:px-8'>
      <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0 mb-4 sm:mb-6 min-w-0'>
        ...
        <Link
          ...
          className='flex items-center justify-center sm:justify-start gap-2 px-3 sm:px-4 py-1.5 sm:py-2 no-underline hover:no-underline bg-foreground/[0.06] hover:bg-foreground/[0.11] rounded-[8px] text-xs sm:text-sm font-medium transition-colors flex-shrink-0'
        >
